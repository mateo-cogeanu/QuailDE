...
- Added tracked helper scripts at `scripts/build-plasma-workspace.sh` and `scripts/install-plasma-dev-session.sh` so Linux builds and dev-session installs are reproducible from the root repo instead of living only in ad-hoc terminal history.
- Fixed the Linux Plasma build bootstrap path so `scripts/build-plasma-workspace.sh` now clones upstream `plasma-workspace` when `vendor/plasma-workspace` is missing, checks out the pinned base revision, and applies `patches/plasma-workspace-quail.patch` automatically instead of assuming the ignored vendor checkout already exists.
- Cut more Plasma shell churn in the local `plasma-workspace` checkout by increasing Kicker runner and app-tree debouncing, stretching shell and scripting config sync intervals, batching waiting-panel creation more aggressively, and shortening the startup phase barrier so a usable session appears faster.
- Reduced panel compositing overhead in `panelview.cpp` by skipping blur and background-contrast work for opaque grounded panels, and by avoiding no-op floating animations when the panel is already at the requested state.
- Updated the root `README.md` and `.gitignore` so the repo now explains the real Linux path for trying the Plasma-based QuailDE customization and keeps local build or vendor noise out of root Git status.
